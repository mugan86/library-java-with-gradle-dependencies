...
import interfaces.RunConverterIF;
 ...
 ****************************************************/
public class Run implements RunConverterIF {
     ...
     */
    @Override
    public String KilometersPerHourToPaceMinKm(double speed_km_h) {
        //Get pace minutes and seconds
        double min_sc_km = 60 / speed_km_h;

        //Convert String
        String result_str = String.valueOf(min_sc_km);

        //Get decimal value
        int index = result_str.indexOf(".");

        //Get only pace minutes
        int min_pace = removeDecimalValue(min_sc_km);
        //Get only pace seconds
        int sec_pace = removeDecimalValue(Double.parseDouble(result_str.substring(index)) * 60);
     ...
     */
    @Override
    public String PaceMinKmToKilometersPerHour(int min_pace, int sec_pace) {
        ...
        double total_time = ((double) sec_pace / 60) + min_pace;
     ...
     */
    @Override
    public String MetersMinuteToKilometersPerHour(double speed_m_min) {
    ...
    public String TotalKilometersAndPacePerKmToTime(double km, String pace_min_km) {
        ...
        int total_time_in_seconds = (int) (sgKm * km);

        //Convert total seconds in time format
        int hours = total_time_in_seconds / 3600;
